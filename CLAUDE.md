...
- **제품**: UI 문자열은 `web/src/i18n/`(ko.ts·en.ts, 키 집합 동일)로만
  쓴다. HTML은 `data-i18n`/`-title`/`-ph` 키를, TS는 `t(key, vars)`를 쓰고
  소스에 UI 리터럴을 새로 넣지 않는다 (주석 제외). 예외는 **글꼴 패밀리명**
  — 문서 데이터라 어느 로케일에서도 리터럴로 둔다. 로케일은
  `?lang=` → localStorage → `navigator.language` 순으로 정해진다.
...
- 상태줄 앞부분(`caret: … | last op … (n/m pages redrawn)`)은 번역하지
  않는다. 제품 문구가 아니라 진단이고 `hf-test.js`가 `S.lastDelta`를
  정규식으로 파싱하는 테스트 계약이다. 상태바를 다시 설계할 때 함께 정리.
