...
- 역매핑은 F-X037 소스 맵 기반. 6개 스토리 전부 텍스트 편집, Enter/병합은
  ...
  미지원.
- 문단 삽입/삭제(Enter/병합)는 본문 블록 수를 바꿔 재시작 레코드를 못 쓰고
  전체 페이지네이션으로 떨어진다 — v0.8 대비 페이지네이션 20–25 → 52–60 ms
  (브라우저 Enter 2.32×). 각주 쪽은 S61에서 노트 캐시 게이트를 문단 단위로
  좁혀 3.4× → 1.4×로 줄였다. 타이핑은 증분이고 v0.8보다 2.5배 빠르다.
