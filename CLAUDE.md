...
- 업스트림 상태 (2026-08-24): **v0.9.0 출시** (2026-08-23) —
  #23/#39/#40/#41/#42/#43 전부 종결, 릴리스 노트에 우리 이슈·PR
  크레딧. dense-form 7건은 F-X048(fa9c0c9) 강화판으로 수용.
  S55 스파이크 결과 **이행 보류**(기능은 등가·dense-form 포크 커밋
  폐기 가능하나 에디터 hot path 2~3× 회귀) — 포크 브랜치
  `svg-poc-0.9`(b3e0143)와 rdoc 적응 패치로 보존.
  **이슈 #46 게시**(2026-08-24): v0.9.0 타이핑/undo 회귀 수치 +
  F-X048 검증 확인 + MarkedContent 마이그레이션 노트. #39/#42에
  감사·검증 코멘트 게시. → 업스트림 응답 대기.
  **#44 → PR #45**: 여전히 open (v0.9.0 미포함), 리뷰 0 — rodf가
  게시한 폰트 별칭 PR(포크 pr/fontfile-family). **리뷰 대응은
