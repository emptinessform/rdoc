...
- 업스트림 상태 (2026-08-26): **v0.10.1 출시** (v0.10.0은 부분 배포로
  ...
  **문서 전역 적격 게이트**(각주·미주·머리글 없음 + 모든 문단
  `lines.len() <= 2`). bench2 2×2 분리: 표는 무관, **문단 길이가 부호를
  뒤집는다** — 1줄 문단이면 v0.10.1이 1.8× 빠르고(4~5 vs 7~9 ms),
  4줄이면 1.3× 느리다. 각주가 생기면 노트 연산이 54~89 → 724~1334 ms. 포크 `svg-poc-0.10`, rdoc `s56-v0101-spike`,
