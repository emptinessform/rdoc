...
## S6-2: 머리글/꼬리글 편집 (포크 c45e271)

- 포크 API `with_header_footer_paragraph_mut`(rel id 주소, 파스→수정→
  재직렬화, 파트 이미지 rels 보존) + 읽기 카운터파트.
- rdoc: 문단 편집 코어(텍스트/범위 삭제/토글)를 `Paragraph` 핸들 자유
  함수로 추출, `EditPath { Doc | HeaderFooter }` 디스패치로 본문·셀·
  머리글이 동일 연산 공유. JS는 `d/`·`h/`·`f/` 경로 편집 허용
  (`fn/`,`en/`은 후속). demo 문서에 머리글+쪽번호 꼬리글 추가.
- 검증 (헤드리스, 7조 전부 통과): 머리글 타이핑/백스페이스/undo/IME
  (`h/rId1/0`), 꼬리글 리터럴 편집(`f/rId2/0`), 쪽번호 필드 글리프는
  비귀속이라 캐럿 거부 + 문서 무손상, 머리글 편집 시 전 페이지
  갱신(1페이지 데모 "1/1"), 본문 회귀. 셀/IME/프리에딧 스위트 그린.
  네이티브 35ms, 테스트 7/7, 코퍼스 14/14.
  스크린샷: `docs/evidence/s7-header-edit.png` (머리글 "[수정됨]").

## 알려진 한계 (백로그)

- 셀 내 Enter(문단 분할)·셀 간 선택 편집 미지원 (명시 거부).
- 각주/미주 편집 미지원 (`fn/`,`en/` 경로는 표시됨). 머리글 내 Enter/
  문단 추가도 미지원 (단일 문단 편집만).
